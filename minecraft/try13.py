...
class Voxel(Button):
    def __init__(self, position=(0, 0, 0), texture=textures['grass']):
        ...
        self.enabled = True  # Initially disable rendering
    ...
    def disable(self):
        if self.enabled:
            self.enabled = False
            self.visible = False  # Set the voxel to be invisible


    def enable(self):
        if not self.enabled:
            self.enabled = True
            self.visible = True  # Set the voxel to be visible

def update_voxels():
    for voxel in voxels.values():
        voxel.update()

player_model = Entity(model='assets/steve.obj', color=color.orange)
player_model.scale=0.5

player = FirstPersonController(model=player_model,position=(0, 15, 0))  # Adjust the starting position of the player

# Pre-generate terrain
for y in range(-3, 10):  # Adjust the range to include blocks both above and below the player
